...
import static dk.apaq.orderly.model.SecurityQuestionType.None;
import dk.apaq.orderly.repository.AccountRepository;
import dk.apaq.orderly.common.security.SecurityHelper;
import dk.apaq.orderly.security.AccountContextHolder;
import dk.apaq.orderly.security.TokenGenerator;
...
public class AccountService extends BaseService<Account, AccountRepository> {
    ...
    public Page<Account> findAllByUnit(String unitId, Pageable pageable) {
        ...
        return getRepository().findAllByRolesContaining(pageable, Roles.generateUnitAccessRole(unitId, Roles.UNITROLE_ADMIN),
                Roles.generateUnitAccessRole(unitId, Roles.UNITROLE_USER));
    }
    ...
    public Iterable<Account> findAllByUnit(String unitId) {
        ...
        return getRepository().findAllByRolesContaining(Roles.generateUnitAccessRole(unitId, Roles.UNITROLE_ADMIN),
                Roles.generateUnitAccessRole(unitId, Roles.UNITROLE_USER));
    }
    ...
    public Account findOne(String id) {
        ...
        if("current".equals(id)) {
            return AccountContextHolder.getCurrent();
        } else {
    ...
    private Account doSave(Account entity) {
        ...
        if(newEntity) {
            ...
            Account creatingUser = AccountContextHolder.getCurrent();
            if(creatingUser != null) {
                for(String role : creatingUser.getRoles()) {
                    if(Roles.isUnitRole(role)) {
                        String orgId = Roles.getUnitId(role);
                        connectToUnit(entity.getId(), orgId, Roles.UNITROLE_USER);
                    }
                }
            }
    ...
    public void changePassword(String currentPassword, String newRawPassword) {
        Account person = AccountContextHolder.getCurrent();
